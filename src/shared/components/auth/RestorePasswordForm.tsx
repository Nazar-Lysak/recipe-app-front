...
const RestorePasswordForm = () => {
  ...
  const [password, setPassword] = useState<PasswordInterface>({pass1: "", pass2: ""});
    ...
    <form className={style.container} onSubmit={submit}>
      ...
      <div className={style.buttonWrapper}>
        <Button type="submit" disabled={!buttonSubmitValidate()}>
          {restorePasswordMutation.isPending ? t('restorePassword.loading') : t('restorePassword.submitButton')}
        </Button>
      </div>

      <AnimatePresence>
        {restorePasswordMutation.isPending && <PagePrealoader variant={"transparent"} />}
      </AnimatePresence>
      {restorePasswordMutation.isError && (
      ...
      {restorePasswordMutation.isSuccess && (
        <Popup onClose={() => {}} isOpen={true}>
          ...
          <CheckIcon />
          <Link to="/login">
            {t('restorePassword.success.backToLogin')}
          </Link>
        </Popup>
      )}
    </form>
